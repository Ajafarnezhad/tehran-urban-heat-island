"""Build interactive maps (geemap/folium) and static charts (matplotlib) for reporting."""
import geemap
...
def make_interactive_map(aoi, lst_baseline, lst_recent, worldcover, center_zoom=11):
    ...
    m.addLayer(worldcover.randomVisualizer(), {}, "Land cover (ESA WorldCover)")
...
def plot_lst_histogram(gdf_baseline: pd.DataFrame, gdf_recent: pd.DataFrame, out_path: str):
    ...
    sns.kdeplot(gdf_baseline["LST_C"], label="Summer 2015", fill=True, ax=ax)
    sns.kdeplot(gdf_recent["LST_C"], label="Summer 2024", fill=True, ax=ax)
    ax.set_xlabel("Land Surface Temperature (°C)")
    ax.set_ylabel("Density")
    ax.set_title("Land Surface Temperature Distribution in Tehran — Period Comparison")
    ax.legend()
    fig.tight_layout()
...
def plot_ndvi_lst_scatter(gdf: pd.DataFrame, out_path: str):
    ...
    ax.set_title("Relationship Between Vegetation (NDVI) and Land Surface Temperature")
    fig.tight_layout()
    fig.savefig(out_path, dpi=200)
    plt.close(fig)
...
def plot_hotspot_map(gdf: gpd.GeoDataFrame, out_path: str):
    ...
    ax.set_title("Hot and Cold Thermal Clusters in Tehran (Getis-Ord Gi* statistic)")
    ax.set_axis_off()
    fig.tight_layout()
    fig.savefig(out_path, dpi=200)
    plt.close(fig)
